...
function MainMenuState:update(dt)
  ...
  GuiManager:addImage(Vector2d(37, 87), "res/gfx/titel2.bmp")

  -- if GuiManager:addButton(Vector2d(434, 300), "online game") then
  --   app.state:switchState(OnlineSearchState())
  -- end

  -- if GuiManager:addButton(Vector2d(434, 340), "lan game") then
  --   app.state:switchState(LANSearchState())
  -- end

  if GuiManager:addButton(Vector2d(434, 380), "start") then
    app.state:switchState(LocalGameState())
  end

  -- if GuiManager:addButton(Vector2d(434, 420), "options") then
  --   app.state:switchState(OptionState())
  -- end

  -- if GuiManager:addButton(Vector2d(434, 460), "watch replay") then
  --   app.state:switchState(ReplaySelectionState())
  -- end

  -- if GuiManager:addButton(Vector2d(434, 500), "credits") then
  --   app.state:switchState(CreditsState())
  -- end

  if GuiManager:addButton(Vector2d(434, 540), "exit") then
    love.event.quit()
  end
end
